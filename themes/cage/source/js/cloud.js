...
export function CloudMask(parentDom, canvasStyle) {
    ...
    function init() {
        ...
        const pageHeight = Math.min(parentDom.clientHeight,window.screen.availHeight);
        // 透视相机，只有距离相机1~500的物体才可以被渲染
        camera = new THREE.PerspectiveCamera(45, pageWidth / pageHeight, 1, 200);
        // 相机的位置，平移下左右平衡
        ...
        const FogColor = '#fff';
        const fog = new THREE.Fog(FogColor, 1, 1000);
        ...
        const texture = new THREE.TextureLoader().load('/images/cloud.png');
